...
for i=1:1:rowNum-1  
    for j=(i+1):1:rowNum
        ...
        switch flag
            ...
            case 2
                if (abs(mod(abs((angle(i,1)-angle(j,1))),360))>thresh && abs(mod(abs((angle(i,1)-angle(j,1))),360))<(180-thresh))||(abs(mod(abs((angle(i,1)-angle(j,1))),360))>(180+thresh) && abs(mod(abs((angle(i,1)-angle(j,1))),360))<(360-thresh))
                    %确定垂直的那条线，求解出交点
        ...
        % 这里添加上对直线交点的筛选条件：交点必须在基站坐标的几何图形内部
        %if pointsX(len)>0
         %   if pointsX(pointsX(len)+1)>=max(Xposition)||pointsX(pointsX(len)+1)<=min(Xposition)||pointsY(pointsY(len)+1)>=max(Yposition)||pointsY(pointsY(len)+1)<=min(Yposition)
          %      pointsX(pointsX(len)+1)=0;pointsY(pointsY(len)+1)=0;
           %     pointsX(len)=pointsX(len)-1;pointsY(len)=pointsY(len)-1;
            %end
        %end
    end
end

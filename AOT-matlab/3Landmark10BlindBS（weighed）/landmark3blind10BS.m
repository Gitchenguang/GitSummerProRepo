...
% 这里进行迭代时，Landmark BSC与Blind BSC 分别对未定位的Blind BSC 定位，加权得到估计位置
% 2014 08 17 10:49 修改了BSC有位置误差时直接使用估计位置作为辐角的估计的错误，已改成用真实坐标估计辐角
...
for ind=1:1:loopNum
    ...
    for i=1:1:BlinBSNum
        ...
        % 剔除tmpBSbroadinfo中要定位的Blind BSC的信息
        tmpBSbroadinfo(i,:)=[];
        tmpTruepos(i,:)=[];
        % 剩余基站对欲测Blind BSC进行辐角定位
        LBSbroadinfo(:,angle)=generangle( iBSxy , LBSbroadinfo);
        tmpBSbroadinfo(:,angle)=generangle( iBSxy , tmpTruepos); % 这里估计角时要用真实的坐标，需要修改
        
        % 汇总 Landmark BSC 与剩余Blind BSC的辐角定位信息
        %mergeBSbroadinfo=[LBSbroadinfo;tmpBSbroadinfo];
        
        % 估计欲定位的Blind BSC的位置
        [estimLX,estimLY,BSbanned]=lslocation( LBSbroadinfo );
        [estimBX,estimBY,BSbanned]=lslocation( tmpBSbroadinfo );
        xy=[(10*estimLX+3*estimBX)/13,(10*estimLY+3*estimBY)/13];

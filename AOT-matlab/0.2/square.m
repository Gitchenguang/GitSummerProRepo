...
% 首先对区域划分网格
Aerawidth=70;
step=0.5;Axy=[0,Aerawidth];Bxy=[0,0];Cxy=[Aerawidth,0];Dxy=[Aerawidth,Aerawidth];
...
for i=step/2:step:(Aerawidth-step/2)
    ...
    % 在这两个循环里计算网格中心点定位的平均误差、最大误差
        xy(1,1)=i;xy(1,2)=j;
        [ thetaA,thetaB,thetaC,thetaD]=generangle(xy,Axy,Bxy,Cxy,Dxy);
        [ setX,setY]=filterpoints( thetaA,thetaB,thetaC,thetaD,Axy,Bxy,Cxy,Dxy);
